...
$val_predic100=[];
$val_ac100=[];
for($x=0;$x<count($herojson["ETH"]["next_days"]);$x++){
    
    array_push($date_predic10,$herojson["ETH"]["next_days"][$x]["index"]);
    array_push($val_predic10,$herojson["ETH"]["next_days"][$x]["values"]);
}

$datelast="";

for($x=0;$x<count($herojson["ETH"]["actual"]);$x++){
    
    array_push($date_acval,$herojson["ETH"]["actual"][$x]["time"]);
    array_push($val_predic100,$herojson["ETH"]["actual"][$x]["close"]);
    array_push($val_ac100,$herojson["ETH"]["predicted"][$x]["values"]);
    if($x==count($herojson["ETH"]["actual"])-1){
...
<head>
  ...
  <title>ETHEREUM PRICE PREDICTIONS
</title>
    <meta charset="utf-8">
    <meta http-equiv="x-ua-compatible" content="ie=edge">
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="bradcaump__inner text-center">
                                <h2 class="bradcaump-title">Ethereum Price Predictions</h2>
                                <nav class="bradcaump-inner">
                                    <a class="breadcrumb-item" href="index.php">Home</a>
                                    <span class="brd-separetor">/</span>
                                    <span class="breadcrumb-item active">Ethereum</span>
                                </nav>
                            </div>
                        </div>
                    </div>
...
$herokulink = file_get_contents('https://cryptopredictordjsc.herokuapp.com/');
$fp = fopen('results.json', 'w');
$asx=json_decode($herokulink, true); 
fwrite($fp, json_encode($asx));
fclose($fp);
