...
//fclose($fp);
//echo $herokulink;
$json_data_file = file_get_contents('resultsbchbsv.json');
...
$val_predic100=[];
$val_ac100=[];
for($x=0;$x<count($herojson["BCH"]["next_days"]);$x++){
    
    array_push($date_predic10,$herojson["BCH"]["next_days"][$x]["index"]);
    array_push($val_predic10,$herojson["BCH"]["next_days"][$x]["values"]);
}

$datelast="";

for($x=0;$x<count($herojson["BCH"]["actual"]);$x++){
    
    array_push($date_acval,$herojson["BCH"]["actual"][$x]["time"]);
    array_push($val_predic100,$herojson["BCH"]["actual"][$x]["close"]);
    array_push($val_ac100,$herojson["BCH"]["predicted"][$x]["values"]);
    if($x==count($herojson["BCH"]["actual"])-1){
...
<head>
  ...
  <title>BITCOIN CASH PRICE PREDICTIONS
</title>
    <meta charset="utf-8">
    <meta http-equiv="x-ua-compatible" content="ie=edge">
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="bradcaump__inner text-center">
                                <h2 class="bradcaump-title">Bitcoin Cash Price Predictions</h2>
                                <nav class="bradcaump-inner">
                                    <a class="breadcrumb-item" href="index.php">Home</a>
                                    <span class="brd-separetor">/</span>
                                    <span class="breadcrumb-item active">BitcoinCash</span>
                                </nav>
                            </div>
                        </div>
                    </div>
...
if($num>=1 || ($num==0 && $datenotnull==1)){
    $herokulink = file_get_contents('https://cryptopredictbchbsv.herokuapp.com/');
$fp = fopen('resultsbchbsv.json', 'w');
$asx=json_decode($herokulink, true); 
fwrite($fp, json_encode($asx));
fclose($fp);
$refreshit=1;
