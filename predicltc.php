...
//fclose($fp);
//echo $herokulink;
$json_data_file = file_get_contents('resultsltceos.json');
...
$val_predic100=[];
$val_ac100=[];
for($x=0;$x<count($herojson["LTC"]["next_days"]);$x++){
    
    array_push($date_predic10,$herojson["LTC"]["next_days"][$x]["index"]);
    array_push($val_predic10,$herojson["LTC"]["next_days"][$x]["values"]);
}

$datelast="";

for($x=0;$x<count($herojson["LTC"]["actual"]);$x++){
    
    array_push($date_acval,$herojson["LTC"]["actual"][$x]["time"]);
    array_push($val_predic100,$herojson["LTC"]["actual"][$x]["close"]);
    array_push($val_ac100,$herojson["LTC"]["predicted"][$x]["values"]);
    if($x==count($herojson["LTC"]["actual"])-1){
...
<head>
  ...
  <title>LITECOIN PRICE PREDICTIONS
</title>
    <meta charset="utf-8">
    <meta http-equiv="x-ua-compatible" content="ie=edge">
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="bradcaump__inner text-center">
                                <h2 class="bradcaump-title">Litecoin Price Predictions</h2>
                                <nav class="bradcaump-inner">
                                    <a class="breadcrumb-item" href="index.php">Home</a>
                                    <span class="brd-separetor">/</span>
                                    <span class="breadcrumb-item active">LTC</span>
                                </nav>
                            </div>
                        </div>
                    </div>
...
if($num>=1 || ($num==0 && $datenotnull==1)){
    $herokulink = file_get_contents('https://cryptopredictltceos.herokuapp.com/');
$fp = fopen('resultsltceos.json', 'w');
$asx=json_decode($herokulink, true); 
fwrite($fp, json_encode($asx));
fclose($fp);
//echo $herokulink;
