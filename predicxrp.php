...
$val_predic100=[];
$val_ac100=[];
for($x=0;$x<count($herojson["XRP"]["next_days"]);$x++){
    
    array_push($date_predic10,$herojson["XRP"]["next_days"][$x]["index"]);
    array_push($val_predic10,$herojson["XRP"]["next_days"][$x]["values"]);
}

$datelast="";

for($x=0;$x<count($herojson["XRP"]["actual"]);$x++){
    
    array_push($date_acval,$herojson["XRP"]["actual"][$x]["time"]);
    array_push($val_predic100,$herojson["XRP"]["actual"][$x]["close"]);
    array_push($val_ac100,$herojson["XRP"]["predicted"][$x]["values"]);
    if($x==count($herojson["XRP"]["actual"])-1){
...
<head>
  ...
  <title>RIPPLE PRICE PREDICTIONS
</title>
    <meta charset="utf-8">
    <meta http-equiv="x-ua-compatible" content="ie=edge">
                    ...
                    <div class="row">
                        <div class="col-lg-12">
                            <div class="bradcaump__inner text-center">
                                <h2 class="bradcaump-title">Ripple Price Predictions</h2>
                                <nav class="bradcaump-inner">
                                    <a class="breadcrumb-item" href="index.php">Home</a>
                                    <span class="brd-separetor">/</span>
                                    <span class="breadcrumb-item active">Ripple</span>
                                </nav>
                            </div>
                        </div>
                    </div>
...
if($num>=1 || ($num==0 && $datenotnull==1)){
    $herokulink = file_get_contents('https://cryptopredictxrpusdt.herokuapp.com/');
$fp = fopen('resultsxrpusdt.json', 'w');
$asx=json_decode($herokulink, true); 
fwrite($fp, json_encode($asx));
fclose($fp);
$refreshit=1;
